    ...
    *error = SUCCESS;
    ...
    while (t && t->type != TOK_END && t->type != TOK_EOF)
    {
        ...
        else                                                                // связываем через узел разделитель
        {
            ...
            if (sep_res.error != SUCCESS)   
            {
                *error = TREE_CREATING_NODE_ERROR;
                return NULL;
            }
